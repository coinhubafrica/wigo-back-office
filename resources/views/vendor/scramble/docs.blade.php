...
<head>
    ...
    <style>
        ...
        /*
         * Bandeau de bascule entre les deux documents publiés (`default` :
         * contrat REST, `realtime` : évènements WebSocket). Stoplight
         * Elements ne sait pas naviguer entre deux specs séparées — sa barre
         * latérale ne liste que les tags d'un seul document — donc le lien
         * vit hors du composant plutôt que dans son arbre de navigation.
         */
        #docs-switcher {
            display: flex; align-items: center; gap: .75rem;
            padding: .5rem 1rem; font: 13px/1.4 -apple-system, sans-serif;
        ...
        #docs-switcher a:hover { opacity: 1; text-decoration: underline; }
        #docs-switcher a[aria-current="page"] { opacity: 1; font-weight: 600; }
    </style>
</head>
<body style="height: 100vh; overflow-y: hidden; display: flex; flex-direction: column;">
<nav id="docs-switcher">
    <a href="{{ url('docs/api') }}" @if($config->name === 'default') aria-current="page" @endif>Contrat REST</a>
    <a href="{{ url('docs/api/realtime') }}" @if($config->name === 'realtime') aria-current="page" @endif>Temps réel (WebSocket)</a>
    <a href="{{ url('docs/api/realtime/flutter') }}" @if($config->name === 'realtime-flutter') aria-current="page" @endif>Client Flutter</a>
</nav>
<elements-api
    id="docs"
