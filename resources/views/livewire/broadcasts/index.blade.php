...
<div>
    <div class="flex flex-wrap items-center gap-3">
        <div>
            <p class="text-sm font-semibold text-ink">{{ __('backoffice.broadcasts.title') }}</p>
            <p class="mt-0.5 text-xs text-muted">{{ __('backoffice.broadcasts.hint') }}</p>
        </div>
    ...
    <div class="mt-4 overflow-hidden rounded border border-line bg-card">
        <div class="overflow-x-auto">
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-surface">
                        <th class="border-b border-line px-4 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-wide text-muted">{{ __('backoffice.broadcasts.column_broadcast') }}</th>
                        <th class="border-b border-line px-4 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-wide text-muted">{{ __('backoffice.broadcasts.column_audience') }}</th>
                        <th class="border-b border-line px-4 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-wide text-muted">{{ __('backoffice.broadcasts.column_status') }}</th>
                        <th class="border-b border-line px-4 py-2.5 text-right text-[10.5px] font-semibold uppercase tracking-wide text-muted">{{ __('backoffice.broadcasts.column_recipients') }}</th>
                        <th class="border-b border-line px-4 py-2.5 text-right text-[10.5px] font-semibold uppercase tracking-wide text-muted">{{ __('backoffice.broadcasts.column_read') }}</th>
                        <th class="border-b border-line px-4 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-wide text-muted">{{ __('backoffice.broadcasts.column_author') }}</th>
                        <th class="border-b border-line px-4 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-wide text-muted">{{ __('backoffice.broadcasts.column_date') }}</th>
                ...
                <tbody>
                    @forelse ($broadcasts as $broadcast)
                        {{-- `wire:key` obligatoire : la liste se réordonne dès qu'une
                             diffusion part, et le diff DOM recyclerait les lignes. --}}
                        <tr wire:key="broadcast-{{ $broadcast->id }}">
                            <td class="border-b border-line px-4 py-3">
                                <b class="block max-w-[280px] truncate text-[13px] text-ink">{{ $broadcast->title }}</b>
                                <div class="mt-0.5 max-w-[280px] truncate text-xs text-muted">{{ $broadcast->body }}</div>
                            </td>
                            <td class="border-b border-line px-4 py-3">
                                <span class="rounded-full px-2.5 py-1 text-[11px] font-semibold {{ $broadcast->audience->badgeClasses() }}">
                                    {{ $broadcast->audience->label() }}
                                </span>
                            </td>
                            <td class="border-b border-line px-4 py-3">
                                <span class="rounded-full px-2.5 py-1 text-[11px] font-semibold {{ $broadcast->status->badgeClasses() }}">
                                    {{ $broadcast->status->label() }}
                                </span>
                            </td>
                            <td class="whitespace-nowrap border-b border-line px-4 py-3 text-right text-[13px] font-semibold text-ink">
                                {{ number_format($broadcast->recipients_count, 0, ',', ' ') }}
                            </td>
                            <td class="whitespace-nowrap border-b border-line px-4 py-3 text-right">
                                {{-- Sans destinataire matérialisé le taux n'a pas de
                                     dénominateur : on affiche un tiret plutôt qu'un 0 %
                                     qui se lirait comme un échec de lecture. --}}
                                @if ($broadcast->recipients_count > 0)
                                    <span class="text-[13px] font-semibold text-ink">{{ number_format($broadcast->read_count, 0, ',', ' ') }}</span>
                                    <span class="ml-1 text-[11px] text-muted">{{ number_format($broadcast->readRate(), 1, ',', ' ') }} %</span>
                                @else
                                    <span class="text-[13px] text-muted">—</span>
                                @endif
                            </td>
                            <td class="border-b border-line px-4 py-3 text-[13px] text-ink">
                                {{ $broadcast->createdByUser?->fullName() ?? __('backoffice.broadcasts.unknown_author') }}
                            </td>
                            ...
                            <td class="whitespace-nowrap border-b border-line px-4 py-3 text-right">
                                @if ($broadcast->status === \App\Enums\BroadcastStatus::Draft)
                                    <button wire:click="confirmSend('{{ $broadcast->id }}')"
                                            class="rounded bg-primary px-3 py-1.5 text-xs font-bold text-white hover:bg-primary-hover">
                                        {{ __('backoffice.broadcasts.send') }}
                                    </button>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-10 text-center">
                                <p class="text-sm font-semibold text-ink">{{ __('backoffice.broadcasts.none') }}</p>
                                <p class="mt-1 text-xs text-muted">{{ __('backoffice.broadcasts.none_hint') }}</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @if ($composerOpen)
        <x-modal close="cancelCompose" align="start" max-width="max-w-2xl"
                 :title="__('backoffice.broadcasts.compose_title')">
            <div class="space-y-4 px-5 py-4">
                <div>
                    <label for="broadcast-title" class="mb-1.5 block text-xs font-semibold text-muted">{{ __('backoffice.broadcasts.field_title') }}</label>
                    <input wire:model="title" id="broadcast-title" type="text"
                ...
                <div>
                    <label for="broadcast-body" class="mb-1.5 block text-xs font-semibold text-muted">{{ __('backoffice.broadcasts.field_body') }}</label>
                    <textarea wire:model="body" id="broadcast-body" rows="4"
                ...
                <div>
                    <label for="broadcast-deeplink" class="mb-1.5 block text-xs font-semibold text-muted">{{ __('backoffice.broadcasts.field_deeplink') }}</label>
                    <input wire:model="deeplink" id="broadcast-deeplink" type="text" placeholder="wigo://recharge"
                           class="block w-full rounded border border-input px-3 py-2 font-mono text-sm placeholder:text-muted focus:border-primary">
                    <p class="mt-1 text-xs text-muted">{{ __('backoffice.broadcasts.deeplink_hint') }}</p>
                    @error('deeplink') <p class="mt-1 text-sm text-err-text">{{ $message }}</p> @enderror
                </div>

                <div>
                    <p class="mb-1.5 text-xs font-semibold text-muted">{{ __('backoffice.broadcasts.audience_label') }}</p>
                    <div class="flex flex-wrap gap-1.5">
                        {{-- `aria-pressed` : la cible retenue n'est signalée que par la
                             couleur, invisible pour un lecteur d'écran sans cet état. --}}
                        @foreach (\App\Enums\BroadcastAudience::cases() as $case)
                ...
                @if ($audience === \App\Enums\BroadcastAudience::Segment->value)
                    <div class="rounded border border-line bg-surface p-4">
                        <p class="mb-1.5 text-xs font-semibold text-muted">{{ __('backoffice.broadcasts.segment_status_label') }}</p>
                        <div class="flex flex-wrap gap-1.5">
                        ...
                        <p class="mb-1.5 mt-4 text-xs font-semibold text-muted">{{ __('backoffice.broadcasts.segment_vehicle_label') }}</p>
                        <div class="flex flex-wrap gap-1.5">
                            ...
                            <button type="button" wire:click="$set('segmentHasVehicle', null)"
                                ...
                                {{ __('backoffice.broadcasts.vehicle_any') }}
                            </button>
                            <button type="button" wire:click="$set('segmentHasVehicle', true)"
                                ...
                                {{ __('backoffice.broadcasts.vehicle_with') }}
                            </button>
                            <button type="button" wire:click="$set('segmentHasVehicle', false)"
                                ...
                                {{ __('backoffice.broadcasts.vehicle_without') }}
                            </button>
                        </div>
                    </div>
                @endif

                @if ($audience === \App\Enums\BroadcastAudience::Individual->value)
                    <div class="rounded border border-line bg-surface p-4">
                        <label for="broadcast-driver-search" class="mb-1.5 block text-xs font-semibold text-muted">{{ __('backoffice.broadcasts.driver_search_label') }}</label>
                        <input wire:model.live.debounce.400ms="driverSearch" id="broadcast-driver-search" type="search"
                               placeholder="{{ __('backoffice.broadcasts.driver_search_placeholder') }}"
                        ...
                        <div class="mt-2 space-y-1">
                            ...
                            @empty
                                <p class="px-1 py-2 text-xs text-muted">
                                    {{ $driverSearch === ''
                                        ? __('backoffice.broadcasts.driver_search_hint')
                                        : __('backoffice.broadcasts.driver_search_empty') }}
                                </p>
                            @endforelse
                        </div>

                        <p class="mt-2 text-xs font-semibold text-muted">
                            {{ trans_choice('backoffice.broadcasts.selected_drivers', count($driverIds), ['count' => count($driverIds)]) }}
                        </p>
                ...
                <div class="rounded border border-primary bg-primary-tint px-4 py-3">
                    <p class="text-2xl font-semibold text-primary-text">
                        {{ trans_choice('backoffice.broadcasts.recipient_count', $recipientCount, ['count' => number_format($recipientCount, 0, ',', ' ')]) }}
                    </p>
                    <p class="mt-0.5 text-xs text-muted">{{ __('backoffice.broadcasts.recipient_count_hint') }}</p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap justify-end gap-2.5 border-t border-line px-5 py-4">
                <button wire:click="cancelCompose" class="rounded border border-line bg-card px-3.5 py-2 text-sm font-semibold text-muted hover:bg-surface">
                    {{ __('backoffice.broadcasts.cancel') }}
                </button>
                <button wire:click="saveDraft" class="rounded border border-line bg-surface px-3.5 py-2 text-sm font-semibold text-ink hover:bg-line">
                    {{ __('backoffice.broadcasts.save_draft') }}
                </button>
    ...
    @if ($confirmingSendId !== null)
        <x-modal close="cancelSend" max-width="max-w-md"
                 :label="__('backoffice.broadcasts.confirm_send_title')">
            <div class="px-5 pb-4 pt-5">
                <p class="text-sm font-semibold text-ink">{{ __('backoffice.broadcasts.confirm_send_title') }}</p>
                {{-- `confirmingCount` et non `recipientCount` : la
                     confirmation porte sur la diffusion réellement envoyée,
                     pas sur l'état du composeur. --}}
                <p class="mt-2 text-xl font-semibold text-primary-text">
                    {{ trans_choice('backoffice.broadcasts.recipient_count', $confirmingCount ?? 0, ['count' => number_format($confirmingCount ?? 0, 0, ',', ' ')]) }}
                </p>
                <p class="mt-1.5 text-sm text-muted">{{ __('backoffice.broadcasts.confirm_send_body') }}</p>
            </div>
