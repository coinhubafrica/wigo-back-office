...
**One row per payment, not per month.** A month is often settled in instalments, so `POST /cnps/declarations` never returns 409 on a period already declared — the handoff spec's 409 contradicts this and was dropped. `is_carry_over` / `source_period` are also omitted **as columns**: settling August's shortfall in September is just `period=2026-08` with a September `payment_date`. Note this is about *storage* only — an excess paid beyond a month's reference **is** carried forward onto the next month, computed at read time by `CnpsStatementService::allocateWithCarryOver()`. See `.ai/rules/livewire-cnps.md`.
