...
Ni `whereDate()` ni `DATE(completed_at)` : même effet, l'index est écarté. Corollaire découvert depuis, et tout aussi contre-intuitif : **trois comptes épinglés par statut battent un `GROUP BY status`** d'un facteur soixante-six. Mesuré sur 2,35 M de courses, pour une seule journée — `GROUP BY status` + `BETWEEN` : 8 409 ms ; `where('status','complete')` + `BETWEEN` + `count()` : 100 ms ; `cancelled` : 65 ms ; les trois ensemble : 127 ms. Un `GROUP BY` ne peut pas épingler `status`, donc le plan retombe sur un parcours complet de l'index. `YangoDailyStatsRecorder` compte statut par statut pour cette raison, et un test le verrouille. `withParticipantsCount()` hydrate l'attribut en `afterQuery()`, donc après pagination, sur les seules lignes visibles ; `ChallengeParticipantsCountTest` garde le contrat avec une assertion de journal de requêtes vide.
