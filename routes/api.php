...
/*
|--------------------------------------------------------------------------
| Domaine — support.wigo.ci
|--------------------------------------------------------------------------
|
| Toute l'API vit sur le domaine du back-office ; seule la vitrine occupe
| `wigo.ci` (cf. `routes/web.php`). En local la contrainte tombe
| (`config('wigo.domains.back_office')` vaut `null`) et l'API répond sur
| n'importe quel hôte.
|
*/
...
Route::domain(config('wigo.domains.back_office'))->group(function (): void {
    ...
    */
    ...
    //
    // ATTENTION : cette URL est enregistrée dans le tableau de bord Wave, par
    // compte — pas dans le code. Elle doit viser le domaine du back-office
    // AVANT toute bascule de domaine, sinon les rappels répondent 404 : Wave
    // tient la notification pour échouée et les paiements cessent d'être
