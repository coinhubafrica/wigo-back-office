...
it('returns the envelope and only active parts in the catalogue', function (): void {
...
it('caps the catalogue page size at fifty', function (): void {
    Sanctum::actingAs(Driver::factory()->create(), ['mobile:*']);
...
it('keeps the models parts and the universal ones when filtering on fits my vehicle', function (): void {
    $dzire = vehicleModel('SUZUKI', 'Dzire');
    $corolla = vehicleModel('TOYOTA', 'Corolla');
...
it('falls back to the whole catalogue without a vehicle when filtering on fits my vehicle', function (): void {
    Sanctum::actingAs(Driver::factory()->create(), ['mobile:*']);
...
it('lets a driver place a pickup order', function (): void {
    $driver = Driver::factory()->create();
    Sanctum::actingAs($driver, ['mobile:*']);
...
it('refuses a pickup order when no agency is active', function (): void {
    $driver = Driver::factory()->create();
    Sanctum::actingAs($driver, ['mobile:*']);
...
it('lets a driver list the active pickup points', function (): void {
    Sanctum::actingAs(Driver::factory()->create(), ['mobile:*']);
...
it('falls back to the only active point for a pickup order', function (): void {
    $driver = Driver::factory()->create();
    Sanctum::actingAs($driver, ['mobile:*']);
...
it('lists only their own orders for a driver', function (): void {
    $driver = Driver::factory()->create();
    Sanctum::actingAs($driver, ['mobile:*']);
...
it('does not find another drivers order', function (): void {
    Sanctum::actingAs(Driver::factory()->create(), ['mobile:*']);
...
it('carries its lines on an order detail', function (): void {
    $driver = Driver::factory()->create();
    Sanctum::actingAs($driver, ['mobile:*']);
...
function vehicleModel(string $brand, string $model): VehicleModel
{
